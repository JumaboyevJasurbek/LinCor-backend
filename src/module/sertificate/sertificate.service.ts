...
export class SertificateService {
  ...
  async createS1(dataCb: any, endCb: any, data: ISertificate): Promise<any> {
    ...
    doc.moveDown(3);
    doc.fontSize(12).fill('#021c27').text('LinCor for Awesomes', {
      align: 'center',
    });
    ...
    doc.image(this.imageURL + '/screen.jpg', 60, 290, {
      valign: 'bottom',
      width: 50,
      height: 50,
    });
  ...
  async createS2(dataCb: any, endCb: any, data: ISertificate): Promise<any> {
    ...
    doc
      .font('Courier-Bold')
      .text(
        'Congratulations for graduating our course and taking your sertificate.',
        {
          align: 'center',
        },
      );
    ...
    doc.moveDown(0.3);
    doc.fillColor('#09597c').fontSize(16).text(`For: ${data.fullname}`);
    ...
    doc.image(this.imageURL + '/logo.jpg', 30, 20, {
      width: 40,
      height: 45,
    });
    doc.image(this.imageURL + '/signature.png', 500, 270, {
      width: 100,
      height: 40,
    });

    doc.image(this.imageURL + '/gold.png', 350, 250, {
      width: 100,
      height: 100,
    });

    doc.moveDown(0.3).fillColor('purple').fontSize(13).text('Lincor', 25, 70);
  ...
  async createS3(dataCb: any, endCb: any, data: ISertificate): Promise<void> {
    ...
    doc.fontSize(16).text('is presented to', { align: 'center' });

    doc.moveDown(0.5);

    doc.font('Times-Italic').fontSize(25).text(data.fullname, {
      align: 'center',
      underline: true,
    });
    ...
    doc
      .fillColor('blue')
      .fontSize(16)
      .text(new Date().toLocaleDateString(), 490, 300, {
        underline: true,
      });

    doc.image(this.imageURL + '/logo.jpg', 330, 20, {
      width: 40,
      height: 45,
    });

    doc.image(this.imageURL + '/gold.png', 300, 280, {
      width: 100,
      height: 100,
    });

    doc.image(this.imageURL + '/screen.jpg', 60, 300, {
      width: 50,
      height: 50,
    });

    doc.fillColor('yellow');
    doc.rotate(15).rect(-20, -30, 140, 420).fill();

    doc.rotate(5).rect(650, -270, 200, 420).fill();
  ...
  async getSertificate(
    ...
    const getCourse: CourseEntity = await CourseEntity.findOne({
      where: { id: courseId },
    });
    ...
    if (getSertificate) {
      throw new HttpException(
        'You can download certificate only once',
        HttpStatus.FORBIDDEN,
      );
    }

    await TakenSertifikat.createQueryBuilder()
      .insert()
